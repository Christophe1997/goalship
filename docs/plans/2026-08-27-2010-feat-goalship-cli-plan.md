...
- **Means:** Replace bash `tk` (wedow/ticket) and goalship's Python `loop_runner.py` with a single Go CLI, `goalship`, and insert a terminal-wizard (TUI) review checkpoint — coordinating with the orchestrating agent purely through the run ledger, no server component — into the execution loop between decomposition and the per-cycle loop.
...
A standalone Go CLI, `goalship`, replaces bash `tk` and the Python `loop_runner.py` mechanics behind goalship's execution loop, installed on PATH like `tk`/`gh`/`glab` today. It adds a pre-loop review checkpoint — a terminal wizard (TUI) — where the user edits and approves the decomposed ticket graph, or rejects it with notes for the agent to regenerate, before any ticket starts executing; the wizard and the agent coordinate entirely through the run ledger, with no server process involved.
...
**Ticket ID scheme**
- R4. New tickets created by `goalship tk create` get an ID shaped `<repo-prefix>-<YYYYMMDD-HHMM>-<4-char-random-suffix>` (e.g. `ae-20260827-1755-05hv`), so a directory listing of `.tickets/` sorts in creation order.
...
**Review checkpoint**
- R9. After a goal is decomposed into a ticket graph and before goalship's execution loop starts its first cycle, `goalship review <run-id>` opens a terminal wizard listing every ticket in the graph. If `<run-id>` doesn't exist, or its ledger `review_state` is already `approved`, the wizard refuses to open and reports a clear error instead — consistent with how preflight failures are reported elsewhere in this system.
- R10. In the wizard, the user can edit any ticket's fields (title, description, acceptance criteria, priority, dependencies) through structured `huh` form fields — a separate editing path from `goalship tk edit`, which stays a bare `$EDITOR` shell-out on the raw file to preserve R1's parity with bash `tk`'s own `cmd_edit`; the two are not unified. Each wizard edit writes straight through to `.tickets/*.md` immediately (no in-memory draft, no separate flush-on-approve step) — except while a rejection is awaiting the agent's regeneration, during which the wizard itself refuses further edits. This is a convention enforced only by the wizard's own read-only mode, not a filesystem lock on `.tickets/*.md`: a `tk create`/`dep` call issued outside the wizard (a stray script, a second agent session) is not blocked by anything at the file level. Deleting or renaming a ticket is out of scope for the wizard, matching bash `tk`'s own command surface, which has no delete operation.
- R11. The user can reject the graph as a whole with notes explaining what's wrong; on reject, `goalship review` writes `review_state: rejected` and the notes to the run ledger via an atomic write, then watches that same ledger file and refreshes the displayed graph in place as soon as the agent's regeneration lands — without the wizard restarting.
- R12. The user can approve the graph as a whole; approval is all-or-nothing across the current ticket set, not per-ticket, and is recorded as `review_state: approved` on the run ledger, together with `approved_ticket_ids` — the exact set of ticket IDs in the graph at the moment of approval. No `goalship` command other than the wizard's approve action ever writes `review_state: approved`; in particular, `goalship loop ledger`'s flag-per-field update surface (R6) never grows a flag that sets `review_state` or `approved_ticket_ids` directly.
- R13. The orchestrating agent discovers a pending rejection by calling `goalship review-status <run-id>` — a plain CLI command, the same pattern already used for every other `goalship`/`tk` interaction — at its own turn boundary, then regenerates the graph using `goalship tk create`/`dep`, and writes `review_state: pending` back to the ledger as the last step of that sequence, after every `tk create`/`dep` call for the regeneration has landed and clearing `review_notes` (they've been addressed) — so the wizard's ledger-driven refresh never surfaces a partially-regenerated graph. The orchestrating skill's own resume-check tests for `review_state: rejected` at every re-entry, not only immediately after decomposition, so a rejection whose regeneration never happened (e.g. the agent's session ended first) is surfaced and re-driven rather than sitting unnoticed indefinitely. This last-step-write ordering is a convention the agent's own prose follows, the same way R10's read-only mode is; nothing in `goalship` enforces it structurally (see Risks & Dependencies for the resulting orphaned-partial-regeneration risk). `goalship review-status` also reports `review_state: approved` (not only `pending`/`rejected`), so a caller never has to special-case approval against a bare absence of rejection.
- R14. `goalship review` refuses a second concurrent invocation against the same run-id — a lock scoped to that run, released on clean exit — so two wizard processes can never edit the same run's tickets at once. Subject to that, the review checkpoint can be entered in the same session immediately after decomposition, or re-entered later in a different session against the same run, as long as that run's ledger `review_state` is not yet `approved` — using goalship's existing run-ledger to identify which run is being resumed.
...
- **The review checkpoint is a terminal wizard (TUI), not the originally-proposed browser page.** (session-settled: user-directed — a terminal wizard has a proven pattern in the sibling `token-profile` repo (the `huh` library, itself built on Bubble Tea), works standalone for a later/resumed review with no server to keep alive, and avoids building a from-scratch local web server with no precedent in either repo.) Governs R9.
...
- F1. **Ticket-graph review**
  ...
  - **Actors:** A1, A2, A3
  - **Steps:** Operator opens `goalship review <run-id>` → wizard lists every ticket → operator edits any tickets (or waits, read-only, while a rejection is pending regeneration) and either approves the whole graph or rejects it with notes → on reject, the wizard writes the rejection to the run ledger; A2 discovers it via `goalship review-status` at its next turn boundary, regenerates the graph via `tk create`/`dep`, and writes the result back to the ledger → the wizard, watching that same ledger file, refreshes in place → operator repeats until approving.
...
```mermaid
flowchart TB
  A[Goal decomposed into ticket graph] --> B[goalship review opens wizard]
  B --> C{Operator action}
  C -->|Edit tickets| B
  C -->|Reject with notes| D[Wizard writes rejection to run ledger]
  D --> E[Agent polls review-status, regenerates via tk create/dep]
  E --> F[Wizard watches ledger, refreshes in place]
  F --> B
...
- AE1. **Covers R11, R12.** Given a decomposed ticket graph is shown in the review wizard, when the user edits two tickets and rejects the graph with a note explaining a missing dependency, then the wizard writes the rejection to the run ledger, the orchestrating agent discovers it via `goalship review-status` and regenerates the graph incorporating that note, and the wizard — watching the same ledger file — refreshes in place: the user never leaves the wizard process.
- AE2. **Covers R12, R15.** Given the user approves the ticket graph in the wizard, when the execution loop's first cycle begins, then `goalship review` cannot be re-entered for that run for the remainder of the run, regardless of how the run eventually terminates.
...
- AE4. **Covers R9.** Given a run-id the operator mistypes, or one whose graph was already approved, when the operator runs `goalship review <run-id>`, then the wizard refuses to open and reports a clear error rather than showing an empty or misleadingly-editable view.
...
- AE6. **Covers R19.** Given a run's `review_state` is `rejected` with notes recorded, when the operator invokes the wizard's withdraw action before the orchestrating agent has regenerated the graph, then `review_state` moves directly to `pending` with no agent call involved, and the wizard becomes editable and rejectable again.
...
- An MCP server or any other persistent listening process in `goalship` — considered and explicitly dropped once research showed the review/regenerate exchange doesn't need one; all agent↔`goalship` coordination is plain CLI calls plus the run ledger (R18).
- Deleting or renaming tickets through the wizard — out of scope; matches bash `tk`'s own command surface, which has no delete operation at all (R10).
...
- **`fsnotify` reliability (KTD2).** Coalesced or dropped filesystem events, or a `.goalship/` directory on an NFS/SMB mount with no notification support, can leave the wizard on a stale view indefinitely. Mitigated by KTD2's polling fallback.
- Dependency version churn on `gojq`, `fsnotify`, `huh`, and `cobra` — pin versions at implementation time and revisit on a normal Go dependency-hygiene cadence; no plan-specific mitigation beyond that.

### Outstanding Questions

None remaining — both questions the brainstorm deferred to planning (the jq-embedding library and the wizard's field-editing UX) are resolved above (Key Decisions; R10).
...
- Sibling `token-profile` repo (`/Users/shendonglai/code/token-profile`): Cobra `NewXCmd()`/`XDeps` pattern, `internal/atomicfile`, `gitops.ExitError`, `huh`-based terminal wizard — no web-UI precedent found there.
...
- Planning-stage `spec-flow-analyzer`, grounded against `run_state.py`, `preflight.py`, `branching.py`, `execution-loop.md`, `SKILL.md`, and the installed `tk` binary directly (confirming `tk` has no delete command, and that a dangling `deps` reference silently blocks a ticket forever with no error surfaced): surfaced that nothing structurally enforced the approval gate before this pass (R16), that a rejected graph had no revival path if the agent's session ended first (R13), that concurrent wizard invocations and missing/approved run-ids had no defined behavior (R9, R14), and that ticket delete/rename was implied but unaddressed (R10).
...
- KTD2. **Live wizard refresh: `fsnotify` watches the ledger file's parent directory (not the file itself), filtering to the ledger path; `huh` v2's `OptionsFunc`/`DescriptionFunc` bindings, bound to `review_updated_at` rather than the raw ticket data, re-render on change.** The watch starts when the wizard opens and runs for the whole session — not lazily on the first reject — so a regeneration that lands before the operator ever rejects (a resumed session, R14) is still picked up. `review_updated_at` carries enough resolution (at minimum, nanosecond wall-clock or a monotonic counter) that two writes in the same wall-clock second are never mistaken for no change. Because `fsnotify` events can be coalesced or dropped, and a `.goalship/` directory on an NFS/SMB mount may not support notifications at all, the wizard also polls `review_updated_at` on a short fixed interval as a fallback — the watch is a latency optimization, not the sole detection mechanism, so a missed event degrades to poll latency rather than a stuck view. Confirmed versions: `charm.land/huh/v2` v2.0.3 (`Eval[T]` hash-based re-evaluation, demonstrated in its own `dynamic-increment` example), `fsnotify` (requires Go 1.23+). Directory-watching avoids the inode-invalidation problem a rename-based atomic write causes for a file-level watch. Implements the Product Contract's MCP-drop Key Decision. Governs R10, R11.
...
flowchart TB
  subgraph goalship binary
    ...
    CLI --> Wizard[internal/wizard: huh model, fsnotify watch]
    Wizard --> Ticket
    Wizard --> Ledger
...
- **Shared workspace, convention-level exclusion.** A1 (wizard, direct `.tickets/*.md` writes) and A2 (agent, `tk`/ledger writes) both touch the same files. Nothing at the filesystem level prevents them from writing concurrently — R10's read-only mode and R13's last-step ledger write are conventions the wizard and the agent's own prose each follow, not a lock either side enforces on the other (see R10, R13, and the orphaned-partial-regeneration risk below).
...
- **Approach:**
  ...
  2. `edit` shells out to `$EDITOR` on the raw file, matching `tk`'s own `cmd_edit` — kept separate from the wizard's structured editing (R10).
...
- **Test scenarios:**
  ...
  - Two concurrent `goalship review` invocations against the same run-id: the second fails fast naming the lock; killing (not clean-exiting) the holder releases the lock for a later invocation.
...
- **Test scenarios:**
  - `commit` never stages `.tickets/*.md` changes, even with pending wizard edits present.
...
- **Test scenarios:**
  ...
  - A nonexistent run-id errors clearly, consistent with R9's wizard-side behavior for the same case.
- **Verification:** a fixture round-trip proving notes text survives verbatim through the ledger write/read cycle.

### U8. `goalship review` — the TUI wizard

- **Goal:** the interactive review checkpoint itself.
- **Requirements:** R9, R10, R11, R12, R14, R19
- **Dependencies:** U2, U5, U7 (the wizard's ledger-read path reuses U7's, not a duplicate)
- **Files:** `internal/wizard/model.go`, `watch.go`, `internal/cli/review.go`, and matching `_test.go` files
- **Approach:**
  1. Open: resolve `<run-id>`'s ledger; refuse if missing or already `approved` (R9); acquire U5's lock (R14, KTD4); list every ticket.
  2. Edit: `huh` fields bound to U2's storage, writing straight through immediately (R10); no delete/rename action offered.
  3. Reject: write `review_state: rejected` + notes atomically (R11); go read-only; offer a withdraw action that writes `review_state: pending` directly (R19), for when the operator wants to undo the rejection without agent involvement; the `fsnotify` watch (KTD2) is already running.
  4. Live refresh: on a `review_updated_at` change, re-read ticket state and update the bound `huh` fields (KTD2) — see the Planning Contract's state diagram.
  5. Approve: write `review_state: approved` plus `approved_ticket_ids` (the current ticket ID set) (R12); release the lock; exit.
  6. Clean exit without a decision: release the lock, leave `review_state` unchanged; a later invocation reopens against current state.
- **Patterns to follow:** `token-profile`'s existing `huh`-based wizard for form structure; this plan's `dynamic-increment` research for the binding mechanism.
- **Test scenarios:**
  - Covers AE1: reject with notes; a separate process writes a regenerated graph to the ledger; the still-running wizard shows the new graph without restarting.
  ...
  - Wizard killed (not clean-exited) mid-review: the lock does not wedge a later invocation.
  - Read-only mode genuinely blocks edits while a rejection is pending regeneration.
  - Withdrawing a rejection from the wizard writes `review_state: pending` directly, with no agent call involved (Covers AE6, R19).
  - Approving writes `approved_ticket_ids` matching exactly the ticket IDs shown in the wizard at approval time (R12).
- **Verification:** a PTY-driven two-process test (the wizard as a real subprocess, a second process performing the reject/regenerate ledger writes) proving live refresh works across real process boundaries — an in-process mock would hide the file-timing bugs this mechanism is most at risk of.
...
- U5 and U8 additionally require the process-boundary tests named in their own Verification fields (claim-gate enumeration, including the `approved_ticket_ids` mismatch case; PTY-driven two-process live-refresh) — an in-process mock is not sufficient evidence for either.
