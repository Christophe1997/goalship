...
// .tickets/ (or a TICKETS_DIR override resolving inside repoRoot) is never
// made git-ignored (that's the target repo's own call), so it still needs
// the explicit negative pathspec below: a ticket's PR must carry only its
// own implementation diff, never this loop's or tk's own bookkeeping
// churn. Mirrors branching.py's commit_all.
func CommitAll(repoRoot, message string) (string, error) {
	...
	if relTicketsDir, ok := ticket.RelativeTicketsDir(repoRoot); ok {
		addArgs = append(addArgs, ":!"+relTicketsDir)
	}
